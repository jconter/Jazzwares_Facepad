...
class GetUserInfo(TestCase):
    ...
    def test_get_userinfo_loggedin(self):
        """Test to make sure that you get user data when
        you request user data when logged in"""

        get_url = reverse(
            "users:get_user", kwargs={"username": self.payload_user["username"]}
        )
        get_user_info = self.client.get(get_url, format="json")
        self.assertEqual(
            get_user_info.data["username"],  # type: ignore
            self.payload_user["username"],
        )
        self.assertEqual(
            get_user_info.data["email"], self.payload_user["email"]  # type: ignore
        )
        self.assertEqual(
    ...
    def test_get_userinfo_notloggedin(self):
        ...
        self.assertEqual(
            get_user_info.status_code, status.HTTP_401_UNAUTHORIZED  # type: ignore
        )

    def test_get_userinfo_friend(self):
        """Test to make sure user can get friend user data"""

        user = get_user_model().objects.get(username=self.payload_user["username"])
        user_friend = get_user_model().objects.get(
            username=self.payload_user_friend["username"]
        )
        user.friends.add(user_friend.id)  # type: ignore
        get_url_friend = reverse(
            "users:get_user", kwargs={"username": self.payload_user_friend["username"]}
        )
        get_user_info = self.client.get(get_url_friend)
        self.assertEqual(
            get_user_info.data["username"],  # type: ignore
            self.payload_user_friend["username"],
        )
        self.assertEqual(
            get_user_info.data["email"], self.payload_user_friend["email"]  # type: ignore
        )
        self.assertEqual(
            get_user_info.data["first_name"],  # type: ignore
            self.payload_user_friend["first_name"],
        )
        self.assertEqual(
            get_user_info.data["last_name"],  # type: ignore
            self.payload_user_friend["last_name"],
        )
        self.assertEqual(
            get_user_info.data["date_of_birth"],  # type: ignore
            self.payload_user_friend["date_of_birth"],
        )

    def test_get_userinfo_notself_orfriend(self):
        ...
        user.friends.add(user_friend.id)  # type: ignore
        get_url_nologin = reverse(
            "users:get_user", kwargs={"username": self.payload_user_nologin["username"]}
        )
        get_user_info = self.client.get(get_url_nologin)
